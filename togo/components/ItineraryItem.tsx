...
export default function ItineraryItem({ id, index, wishlistItem, destName, destDesc, destImg, itemNote, onDelete }: ItineraryItemProps) {
  ...
  function editNote() {
    setIsEditingNote(prev => true);
  }
  function saveNote() {
    setIsEditingNote(prev => false);
            ...
            <div className={`shrink ${snapshot.isDragging ? "bg-gray-100 itinerary-item-dragging" : "bg-gray-50"} rounded-lg border-1 border-dashed overflow-hidden ${wishlistItem ? "w-55 h-18 flex-none" : "w-full min-h-20"}`}>
              ...
              <div className="flex flex-nowrap justify-between items-center h-full">
                ...
                <div {...provided.dragHandleProps}>
                  <svg className="move-icon w-10 grow-0 shrink-0" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M5.14285 8.57144H18.8571M5.14056 12H18.8514M5.14285 15.4286H18.8514" stroke={`${snapshot.isDragging ? "black" : "gray"}`}/>
                  </svg>
                </div>
              </div>
              ...
              <div className="border-t-1 border-dashed overflow-hidden mx-2" hidden={isExpanded ? false : wishlistItem || !itemNote}>
                <div className="flex gap-2 my-1">
                  ...
                  <textarea readOnly={!isEditingNote} onClick={editNote} className={`self-center bg-gray-100 px-3 py-1 rounded-lg field-sizing-content resize-none text-md/5 ${isEditingNote ? "w-full" : "truncate"}`} defaultValue={itemNote} />
